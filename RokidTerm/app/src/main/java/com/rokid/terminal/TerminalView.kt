...
class TerminalView(context: Context) : View(context) {
    ...
    /**
     * AskUserQuestion panel state (real case 2026-08-10). NOT rendered as an
     * overlay — the panel stays embedded in the terminal like the command
     * panels (/effort, /model); the parsed state only drives Type-something
     * detection and the mirrored selection (user decision 2026-08-10).
     */
    private var askPanelOptions: List<AskPanelParser.Option> = emptyList()
    private var askPanelSelected = 0

    /**
     * True while a command/ask panel is open (set by the activity). The
     * blinking "_" input cursor must NOT render while a picker occupies the
     * input line ("❯ 1. 选项…") — only the bare input bar gets the blinking
     * cursor (user 2026-08-10).
     */
    private var panelActive = false

    /** Drives [panelActive] from the activity's panel state machine. */
    ...
    /**
     * Records the parsed AskUserQuestion options + mirrored selection. The
     * panel itself is rendered BY Claude (embedded, like the command
     * panels) — nothing is drawn here; the state drives Type-something
     * confirm and the mirrored selection only.
     */
    fun setAskPanel(options: List<AskPanelParser.Option>, selected: Int) {
    ...
    fun askPanelSnapshot(): AskPanelParser.Snapshot? {
        ...
        return AskPanelParser.Snapshot(
            ...
            multiSelect = options.any { it.checkbox },
            tabPanel = AskPanelParser.isTabPanel(rows),
        )
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        } else if (blinkOn && inputLineText() == null) {
            // Blinking input cursor ONLY on the bare input bar (empty ❯).
            // Any non-empty input line — a picker's echoed selection, a
            // suggestion — must not get our blinking "_" (2026-08-10; the
            // previous panelActive check jittered with the detect/lost
            // state machine and the "_" reappeared).
