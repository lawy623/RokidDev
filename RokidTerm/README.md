...
- **捕获噪声过滤（2026-08-14）**：Claude TUI 的瞬态 UI 行按**内容特征**排除（位置会随版本漂移）：思考/工具状态行（spinner 字符、计时器、`(thinking…)`）、管道符 markdown 表格（流式中间态，收尾才重绘成盒式表格）、`⎿` 工具执行块（命令输入+输出+Tip）；**重绘碎片规则**——被覆盖行的内容仍以相同/更长形式存在于屏幕其他行即视为中间态跳过（❯ 用户行豁免），一行规则消除多世代碎片交错、表格重复；**ticker 守卫（格式无关）**——同一行位置连续被捕获的内容只差数字即判定为活动计时区（首个 tick 捕获、第二个证明模式后全部丢弃；纯数字行如 `seq` 输出不受影响，`●` 标记交替与行首空白归一化处理）；**渲染/历史解耦**——历史过滤用 `isStatusRow`（计时器行绝不进历史），渲染抑制用 `isRenderSuppressible`（只冻结无计时器的纯 spinner/thinking 动画行，计时器每秒照常刷新，避免"看不出工具是否跑完"）。
...
（默认 1 小时，三重信号防误杀，选中窗口永不清扫；App 连接时每 5 分钟自动执行一次）。
...
- 清扫调参：`rokid-sessions sweep <tmux-session> <base-dir> <idle-minutes>`（App 每 5 分钟
  自动调一次，默认 60 分钟；2026-08-14 起空闲判定改用 JSONL 最后一条记录的时间戳而非
  文件 mtime——Claude Code 自身的日志维护重写 JSONL 会刷新 mtime、把死对话的
  空闲时钟清零，内容时间戳不受影响）。
