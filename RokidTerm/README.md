...
- **会话内捕获**：Claude Code 使用重绘式渲染（绝对定位改写、无滚动转义码），处理器通过**基线对比**检测内容上移，把滚出屏幕的对话行推入本地历史（上限 5000 行）。**流式输出（2026-08-13 加固）**：快节奏原地覆盖重绘（如 `seq 1 2000` 刷屏）基线对比全部失效——模拟器级"替换前捕获"在行被覆盖前把稳定内容收进历史（50ms 稳定门槛 + 暂存确认），`seq 1 2000` 完整可浏览。
...
- **重连历史（2026-08-14）**：重连导入的 export 历史与活屏无缝衔接——浏览视图自动跳过与活屏重复的尾部（渲染层重叠检测，不依赖时序）；导入行统一加 2 空格缩进、`❯` 用户消息行恢复深色背景高亮，浏览效果与应用内实时一致。
